...
def sanitize(goals):
    ...
    for g in goals["goals"]:
        ...
        if g.get("status") not in ("active", "completed", "abandoned"):
            g["status"] = "active"
...
def apply_ops(goals, important, ops, max_new_top_level=1):
    ...
    for o in ops or []:
        ...
        elif op == "set_status" and g and o.get("status") in ("active", "completed", "abandoned"):
...
def render(goals, important, show_all=False):
    ...
    def visible(g):
        return show_all or g["status"] == "active"
    ...
    def emit(g, prefix, last, root=False):
        tag = "" if g["status"] == "active" else dim(f" ({g['status']})")
...
def write_goal_context(trajdir: Path, goals, important):
    ...
    items = {i["id"]: i for i in important["items"]}
    def emit(g, ind):
        lines.append(f"{'  ' * ind}- {g['title']} [{g['status']}]")
        for t in g["todos"]:
            if not t["done"]:
    ...
    for g in [x for x in goals["goals"]
              if not x.get("parent_goal_id") and x["status"] == "active"]:
        emit(g, 0)
    txt = "\n".join(lines)[:1900]
