...
class StandardEditorBrowserTests(BrowserCase):
    ...
    def test_notes_save_and_export_include_context_status_and_completed_rows(self):
        ...
        with server_for(self.chat) as url,self.page_on(url) as (page,errors):
            ...
            page.get_by_label('Todo',exact=True).last.press('Control+a')
            self.expect(page.locator('.todo-entry.is-picked')).to_have_count(2)
            page.get_by_label('Todo',exact=True).last.press('Control+a')
            self.expect(page.locator('.todo-entry.is-picked')).to_have_count(2)
            page.get_by_label('Todo',exact=True).last.press('Escape')
            self.expect(page.locator('.todo-entry.is-picked')).to_have_count(0)
            page.get_by_label('New todo').fill('Include this unfinished draft too.')
            ...
            self.assertIn('Include this unfinished draft too.',text)
            page.reload(wait_until='domcontentloaded')
            page.get_by_role('tab',name='Notes',exact=True).click()
